...
const Subject = ({ navigation, token }: MyProps) => {
  ...
  const getSubject = () => {
    axios.get(`${BASE_URL}/mnemonicsub?api_token=${token}`).then(
      (res) => {
        setSubject(res.data.mnemonics)
        setLoading(false)
      },
      (error) => {
        setLoading(false)
      ...
      <View style={styles.content}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              onPress={() =>
                navigation.navigate('Mnemonics', { subject_id: item.id })
              }
              style={styles.boxContainer}
            >
              <Text style={styles.heading}>{item.name}</Text>
              <Text style={styles.subheading}>
                {item.total} mnemonic{item.total > 1 ? 's' : ''}
              </Text>
            </TouchableOpacity>
          )}
