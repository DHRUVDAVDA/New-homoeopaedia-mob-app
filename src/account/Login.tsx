...
const Login = ({ navigation, logIn }: MyProps) => {
	...
	const submitForm = async (email: string, password: string, forceLogin = false) => {
		setLoading(true);
		const fcmToken = await AsyncStorage.getItem('fcm_token');
	...
	const sendFCMToken = async (userid) => {
		const fcmToken = await AsyncStorage.getItem('fcm_token');
		try {
			await axios.post(`${BASE_URL}/store-fcm-token`, { user_id: userid, token: fcmToken }).then(
				(res) => {
	...
	return (
		...
		>
			<Loading
				loading={loading}
				text="Checking your account. Please wait."
			/>
