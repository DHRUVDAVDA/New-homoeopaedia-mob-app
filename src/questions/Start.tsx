...
const Start = ({ navigation, route, token, user }: MyProps) => {
	...
	const getQuestions = () => {
		axios
			.get(
				`${BASE_URL}/qload/${subId}/${chapterId}/${user.user_id}?api_token=${token}`,
			)
			.then(
				(res) => {
	...
	const getCircle = () => {
		axios
			.get(
				`${BASE_URL}/qcircle/${subId}/${chapterId}/${user.user_id}?api_token=${token}`,
			)
			.then(
				(res) => {
	...
	const saveAnswer = (option: number) => {
		...
		axios
			.post(`${BASE_URL}/saveqanswer?api_token=${token}`, {
	...
	const finishQuestion = () => {
		...
		axios
			.post(`${BASE_URL}/completesection?api_token=${token}`, {
				user_id: user.user_id,
				subject_id: subId,
					...
					<View style={styles.contentContainer}>
						...
						{!optselected ? (
							<ScrollView contentContainerStyle={styles.scroller}>
								<HTML
									source={{
										html: `${index + 1}) ${questions?.[index]?.["ques"]
											}`,
									}}
									contentWidth={width}
									tagsStyles={{
