...
const Unattempted = ({ navigation, user, token, subId }: MyProps) => {
	...
	const getUnattempted = () => {
		axios
			.get(
				`${BASE_URL}/unattempted/${subId}/${user.user_id}?api_token=${token}`,
			)
			.then(
				(res) => {
					setUnattempted(res.data.unattempted);
					setFilteredArray(res.data.unattempted);
				},
				(error) => {
	...
	const startSearch = (text: string) => {
		if (text === "") {
			setFilteredArray(unattempted);
		} else {
			const filteredData = unattempted.filter((item: any) =>
		...
		<View style={styles.content}>
			{unattempted.length > 0 ? (
				...
				<View style={styles.scroller}>
					<Text>No unattempted items found</Text>
				</View>
			)}
		</View>
	);
...
export default connect(mapStateToProps)(Unattempted);
